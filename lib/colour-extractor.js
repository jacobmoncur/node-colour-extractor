...
(function() {
  ...
  exports.topColours = function(sourceFilename, sorted, cb) {
    ...
    return img.size(function(err, wh) {
      var h2, height, ratio, square, w2, width, x, y;
      if (err) {
        return cb(err, null);
      }
      ...
      w2 = wh.width / 2;
      h2 = wh.height / 2;
      square = (wh.width < wh.height ? wh.width / 2 : wh.height / 2);
      width = square;
      height = square;
      x = wh.width / 2 - square / 2;
      y = wh.height / 2 - square / 2;
      return img.noProfile().bitdepth(8).crop(width, height, x, y).scale(Math.ceil(wh.height / ratio), MAX_W).write('histogram:' + tmpFilename, function(err) {
        ...
        miffRS.on('error', function(error) {
  ...
  exports.colourKey = function(path, cb) {
    return exports.topColours(path, false, function(err, xs) {
      var M, m;
      if (err) {
        cb(err, null);
      }
